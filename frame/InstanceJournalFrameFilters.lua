...
function IJ_ClassFilter_OnClick()
    ...
    if IJ_ActiveInfoTab == 2 then
        if IJ_IsInstanceTabActive and IJ_SelectedInstance then
...
function IJ_ContinentFilterDropDown_Initialize()
    ...
    for key, contName in pairs(IJLib.Continents) do
        info = {}
        info.text = contName
        info.value = key
        info.func = IJ_ContinentFilter_OnClick
        info.checked = (currentFilter == key)
        UIDropDownMenu_AddButton(info)
    end
end
...
function IJ_ContinentFilter_OnClick()
    ...
    local displayText = IJ_GUI_ALLCONTINENTS

    if this.value ~= "ALL" then
        displayText = IJLib.Continents[this.value] or this.value
    end
...
function IJ_UpdateNavBars()
    ...
    if IJ_CurrentBoss and not IJ_IsInstanceTabActive then
        IJ_BossNavBar:Show()
